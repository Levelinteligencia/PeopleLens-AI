...
| **P-02** | período relativo: resolver contra a cobertura do KPI ou recusar como ambiguidade? | **APROVADA** em 2026-09-13 e **concluída fim a fim** no commit `30cd6ee`: o adapter preserva `{grain, relativo}` e o `RESOLVE` materializa contra a cobertura declarada do KPI (seção 7). D-L1 está fechado |
| **P-03** | **qual modelo**, e por qual provedor | **APROVADA COM LACUNA CONHECIDA**, por avaliação empírica: `gpt-5.6-luna`, da OpenAI. 16 de 17 casos, 4 de 4 em equivalência PT/EN, sem fallback e sem retry. O único caso que falhou expôs uma lacuna do **contrato** (`compare_to` não representa dois períodos arbitrários), e não do modelo. Ver `docs/llm_interpreter_p03_report.md` |
